...
export const GameDetailModal: React.FC<GameDetailModalProps> = ({
  ...
  const localizedSimilarGames = similarGames.map((g) => localize(g));
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.game) {
          ...
          setRegenerateNotice(t("modalRegenerateSuccess"));
      ...
      <div className="relative w-full max-w-4xl bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl overflow-hidden my-auto max-h-[90vh] flex flex-col">
        ...
        <div className="sticky top-0 z-10 bg-zinc-900/95 backdrop-blur border-b border-zinc-800 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-bold text-zinc-100 truncate">{localizedGame.title}</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 space-y-2 text-xs">
                <div className="flex items-center justify-between text-zinc-400">
                  <span className="flex items-center gap-1.5 text-zinc-500">
                    <Building2 className="w-3.5 h-3.5" /> {t("modalDeveloper")}
                  </span>
                  <span className="font-mono text-zinc-200">{localizedGame.developer}</span>
                </div>
                <div className="flex items-center justify-between text-zinc-400">
                  <span className="flex items-center gap-1.5 text-zinc-500">
                    <Calendar className="w-3.5 h-3.5" /> {t("modalReleaseDate")}
                  </span>
                  <span className="font-mono text-zinc-200">{localizedGame.releaseDate}</span>
                </div>
                <div className="pt-2 border-t border-zinc-800/60">
                  <span className="text-[11px] text-zinc-500 block mb-1.5">{t("modalPlatformsAndScores")}</span>
                  <div className="space-y-1.5">
                    {localizedGame.platforms.map((plat) => (
                      <div
                        key={plat.platform}
                ...
                <div className="pt-2">
                  ...
                  >
                    {t("modalViewMetacritic")} <ExternalLink className="w-3 h-3 text-zinc-400" />
                  </a>
                </div>
              </div>
            </div>
            ...
            <div className="md:col-span-2 space-y-6">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {localizedGame.genres.map((genre) => (
                    <span
                      key={genre}
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-xs uppercase tracking-wider text-zinc-400 font-mono flex items-center gap-1.5">
                    <Film className="w-3.5 h-3.5 text-zinc-400" /> {t("modalOfficialTrailer")}
                  </h3>
                  <a
                    href={game.videoUrl}
                    target="_blank"
                  ...
                  >
                    {t("modalWatchYouTube")} <ExternalLink className="w-3 h-3" />
                  </a>
                </div>
                ...
                {trailerEmbed ? (
                  <div className="space-y-2">
                    <div className="aspect-video w-full rounded-xl overflow-hidden bg-black border border-zinc-800 shadow-inner">
                      <iframe
                        src={trailerEmbed}
                        title={`${localizedGame.title} trailer`}
                    ...
                    <div className="flex items-center justify-between px-1 text-[11px] text-zinc-500 font-mono">
                      <span>{t("modalVideoFilterHint")}</span>
                      <a
                        href={game.videoUrl}
                        target="_blank"
                        rel="noreferrer"
                  ...
                  <div className="p-4 rounded-xl bg-zinc-950/60 border border-zinc-800 text-center text-xs text-zinc-400">
                    ...
                    >
                      <PlayCircle className="w-4 h-4 text-red-400" /> {t("modalOpenTrailerStream")}
                    </a>
                  </div>
                )}
              </div>
          ...
          {/* AI Review Summaries Section */}
          <div className="pt-2 border-t border-zinc-800">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
              <div className="flex items-center gap-2">
                <Bot className="w-4 h-4 text-zinc-300" />
                <h3 className="text-sm font-semibold text-zinc-100">
                  {t("modalAiSummariesHeader")}
                </h3>
              </div>
              <button
              ...
              >
                ...
                {isRegenerating ? t("modalRegenerating") : t("modalRegenerateBtn")}
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center justify-between pb-2 border-b border-zinc-800/80">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 text-[10px] font-mono px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-700 text-zinc-300">
                      ...
                      {localizedGame.criticReviewSummary?.model || "Google Gemini 2.5 Flash"}
                    </span>
                    <span className="text-[10px] font-mono text-zinc-500">
                      {localizedGame.criticReviewSummary?.sampleCount ?? 0} {t("modalReviewsCount")}
                    </span>
                  </div>
                </div>
                ...
                {localizedGame.criticReviewSummary ? (
                  <>
                    <p className="text-xs text-zinc-300 italic leading-relaxed">
                      &ldquo;{localizedGame.criticReviewSummary.consensus}&rdquo;
                    </p>

                    <div className="space-y-2 pt-1">
                      <div>
                        <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-medium mb-1">
                          <ThumbsUp className="w-3 h-3" /> {t("modalCriticsPraise")}
                        </div>
                        <ul className="space-y-1 text-xs text-zinc-400 pl-4 list-disc">
                          {localizedGame.criticReviewSummary.liked.map((item, i) => (
                            <li key={i}>{item}</li>
                          ))}
                        </ul>
                      </div>
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 text-xs text-rose-400 font-medium mb-1">
                          <ThumbsDown className="w-3 h-3" /> {t("modalCriticsDislike")}
                        </div>
                        <ul className="space-y-1 text-xs text-zinc-400 pl-4 list-disc">
                          {localizedGame.criticReviewSummary.disliked.map((item, i) => (
                            <li key={i}>{item}</li>
                          ))}
                        </ul>
                      </div>
                  ...
                  <p className="text-xs text-zinc-500">{t("modalNoCriticReviews")}</p>
                )}
              </div>
              ...
              <div className="bg-zinc-950/60 border border-zinc-800 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between pb-2 border-b border-zinc-800/80">
                  <span className="text-xs font-semibold text-zinc-200 uppercase tracking-wider font-mono">
                    {t("modalUserSummaryTitle")}
                  </span>
                  <div className="flex items-center gap-2">
                    <span className="inline-flex items-center gap-1 text-[10px] font-mono px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-700 text-zinc-300">
                      <Sparkles className="w-2.5 h-2.5 text-amber-400" />
                      {localizedGame.userReviewSummary?.model || "Google Gemini 2.5 Flash"}
                    </span>
                    <span className="text-[10px] font-mono text-zinc-500">
                      {localizedGame.userReviewSummary?.sampleCount ?? 0} {t("modalReviewsCount")}
                    </span>
                  </div>
                </div>

                {localizedGame.userReviewSummary ? (
                  <>
                    <p className="text-xs text-zinc-300 italic leading-relaxed">
                      &ldquo;{localizedGame.userReviewSummary.consensus}&rdquo;
                    </p>

                    <div className="space-y-2 pt-1">
                      <div>
                        <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-medium mb-1">
                          <ThumbsUp className="w-3 h-3" /> {t("modalPlayersPraise")}
                        </div>
                        <ul className="space-y-1 text-xs text-zinc-400 pl-4 list-disc">
                          {localizedGame.userReviewSummary.liked.map((item, i) => (
                            <li key={i}>{item}</li>
                          ))}
                        </ul>
                      </div>
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 text-xs text-rose-400 font-medium mb-1">
                          <ThumbsDown className="w-3 h-3" /> {t("modalPlayersDislike")}
                        </div>
                        <ul className="space-y-1 text-xs text-zinc-400 pl-4 list-disc">
                          {localizedGame.userReviewSummary.disliked.map((item, i) => (
                            <li key={i}>{item}</li>
                          ))}
                        </ul>
                      </div>
                  ...
                  <p className="text-xs text-zinc-500">{t("modalNoUserReviews")}</p>
                )}
              </div>
            </div>
          </div>

          {/* YouTube Let's Play Analysis Section */}
          <div className="pt-2 border-t border-zinc-800">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <PlayCircle className="w-4 h-4 text-red-400" />
                <h3 className="text-sm font-semibold text-zinc-100">
                  {t("modalLetsPlayHeader")}
                </h3>
              </div>
              <div className="flex items-center gap-2">
                <span className="inline-flex items-center gap-1 text-[10px] font-mono px-1.5 py-0.5 rounded bg-zinc-900 border border-zinc-700 text-zinc-300">
                  <Sparkles className="w-2.5 h-2.5 text-amber-400" />
                  {localizedGame.letsPlayAnalysis?.model || "Google Gemini 2.5 Flash"}
                </span>
                {localizedGame.letsPlayAnalysis && (
                  <span className="text-[11px] font-mono text-zinc-500">
                    {localizedGame.letsPlayAnalysis.viewCount} &bull; {localizedGame.letsPlayAnalysis.channelName}
                  </span>
                )}
              </div>
            </div>
            ...
            {localizedGame.letsPlayAnalysis ? (
              <div className="bg-zinc-950/60 border border-zinc-800 rounded-xl p-4 space-y-3">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-2 border-b border-zinc-800/80">
                  <div>
                    <h4 className="text-xs font-semibold text-zinc-200">
                      {localizedGame.letsPlayAnalysis.videoTitle}
                    </h4>
                    <span className="text-[11px] text-zinc-500 font-mono">
                      {t("modalChannel")} {localizedGame.letsPlayAnalysis.channelName}
                    </span>
                  </div>
                  <a
                    href={localizedGame.letsPlayAnalysis.videoUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-1 text-xs text-zinc-400 hover:text-zinc-200 font-mono shrink-0"
                  >
                    {t("modalWatchLetsPlay")} <ExternalLink className="w-3 h-3" />
                  </a>
                </div>

                {/* Streamer takeaway */}
                <p className="text-xs text-zinc-300 leading-relaxed">
                  {localizedGame.letsPlayAnalysis.summary}
                </p>
                ...
                <div className="bg-zinc-900/80 border border-zinc-800 px-3 py-2 rounded-lg text-xs text-zinc-300 font-mono italic">
                  {localizedGame.letsPlayAnalysis.bloggerVerdict}
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  <div>
                    <span className="text-[11px] font-mono text-emerald-400 uppercase tracking-wider block mb-1">
                      {t("modalStreamerPositives")}
                    </span>
                    <ul className="text-xs text-zinc-400 space-y-0.5 list-disc pl-4">
                      {localizedGame.letsPlayAnalysis.pros.map((p, i) => (
                        <li key={i}>{p}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
                    <span className="text-[11px] font-mono text-rose-400 uppercase tracking-wider block mb-1">
                      {t("modalStreamerConcerns")}
                    </span>
                    <ul className="text-xs text-zinc-400 space-y-0.5 list-disc pl-4">
                      {localizedGame.letsPlayAnalysis.cons.map((c, i) => (
                        <li key={i}>{c}</li>
                      ))}
                    </ul>
                  </div>
              ...
              <p className="text-xs text-zinc-500">{t("modalNoLetsPlay")}</p>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-zinc-800">
            <h3 className="text-sm font-semibold text-zinc-100 mb-3">
              {t("modalSimilarGamesTitle")}
            </h3>

            {localizedSimilarGames.length === 0 ? (
              <p className="text-xs text-zinc-500">{t("modalNoSimilarGames")}</p>
            ) : (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {localizedSimilarGames.map((sim) => (
                  <div
                    key={sim.id}
